...
function LoginForm() {
  ...
  useEffect(() => {
    fetch('/api/config')
      ...
      .then((c: PublicConfig | null) => setConfig(c))
      .catch(() => setConfig(null));
  }, []);

  async function doSignIn(credentials: { email: string; password: string }) {
    setLoading(true);
    setError('');
  ...
  async function handleDemo(e: FormEvent) {
    e.preventDefault();
    const demo = config?.demo ?? { email: 'demo@leadpilot.io', password: 'demo1234' };
    setEmail(demo.email);
        ...
        <div className='rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl'>
          ...
          <div className='mt-4'>
            <Button
              ...
              disabled={loading}
              onClick={handleDemo}
              className='w-full border-brand-500/40 bg-brand-500/10 text-brand-200 hover:bg-brand-500/20'
            >
              <span className='mr-1.5 text-base leading-none'>⚡</span> Use demo account instantly
            </Button>
            <p className='mt-2 text-center text-xs text-ink-500'>
              Free demo — no registration needed:{' '}
              <span className='font-mono text-brand-300'>demo@leadpilot.io</span>
            </p>
          </div>
